...
export function processQuarter(gameState) {
  ...
  console.log(`[ENGINE START] Q${quarter}, ${teams.length} teams, ${segments.length} segments`);
  console.log(`[ENGINE] decision keys: ${JSON.stringify(Object.keys(decisions))}`);
  for (const team of teams) {
    const d = decisions[team.id];
    const dk = decisions[String(team.id)];
    console.log(`[ENGINE INPUT] team.id=${team.id} (type=${typeof team.id}) hasDec=${!!d} hasDecStr=${!!dk} brands=${team.brands?.length||0}`);
  }
  ...
  for (const segment of segments) {
    ...
    for (const region of regions) {
      ...
      if (totalDemand > 0) console.log(`[ENGINE] seg=${segment.name} reg=${region} potential=${adjustedPotential} demandCreation=${demandCreation.toFixed(3)} totalDemand=${totalDemand}`);
...
function getPriceAttractiveness(pricing, brand, segment) {
  if (!brand || !pricing) { console.log('[PRICE] no brand or pricing, returning 0.5'); return 0.5; }
  ...
  console.log(`[PRICE] brand=${brand.name} price=${price} min=${minPrice} max=${maxPrice} ideal=${idealPrice} sens=${priceSensitivity} rawMin=${segment.min_price} rawMax=${segment.max_price}`);

  // Price within range gets base score; further from ideal = lower score
  if (price < minPrice * 0.7 || price > maxPrice * 1.5) {
    console.log(`[PRICE] OUT OF RANGE: ${price} < ${minPrice*0.7} or ${price} > ${maxPrice*1.5}`);
    return 0.1;
  }
  ...
  console.log(`[PRICE] deviation=${deviation.toFixed(4)} attractiveness=${attractiveness.toFixed(4)}`);
...
function computeTeamResults(team, decisions, brandScores, demandResults, segments, regions, quarter, prevResults) {
  ...
  for (const segment of segments) {
    ...
    for (const region of regions) {
      ...
      const price = parseFloat(brandFit ? (decisions.pricing?.[brandFit.brand.name] || decisions.pricing?.default || 900) : 900);
      const unitCost = parseFloat(brandFit ? (brandFit.brand.unit_cost || 400) : 400);
  ...
  const rdExpense = parseFloat(decisions.rdBudget || 0);
  ...
  // Cash flow (with NaN guards)
  const prevCash = parseFloat(prevResults?.endingCash || team.cash_balance || 5000000) || 5000000;
  const dividend = parseFloat(decisions.dividend) || 0;
  const cashFlow = isNaN(netIncome) ? 0 : netIncome - dividend;
  const endingCash = isNaN(prevCash + cashFlow) ? prevCash : prevCash + cashFlow;
...
function calculateSalesforceExpense(salesforce) {
  ...
  if (salesforce) {
    for (const region of Object.values(salesforce)) {
      ...
      const comp = parseFloat(region?.compensation || 30000);
