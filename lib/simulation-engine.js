...
export function processQuarter(gameState) {
  ...
  for (const team of teams) {
    ...
    console.log(`[ENGINE INPUT] team=${team.id} (${team.name}) brands=${team.brands?.length||0} hasDec=${!!d} dist=${JSON.stringify(d?.distribution)} pricing=${JSON.stringify(d?.pricing)} brandTargets=${team.brands?.map(b=>b.target_segment)}`);
  }
  console.log(`[ENGINE SEGMENTS] ${segments.map(s => s.name).join(', ')}`);
  ...
  for (const segment of segments) {
    ...
    for (const region of regions) {
      ...
      for (const team of teams) {
        ...
        if (targeting <= 0) {
          console.log(`[ENGINE] team=${team.id} seg=${segment.name} reg=${region} SKIP targeting=${targeting} dist=${JSON.stringify(d.distribution?.[region])}`);
          continue;
        }
        ...
        console.log(`[ENGINE] team=${team.id} seg=${segment.name} reg=${region} targeting=${targeting.toFixed(2)} brandFit=${(brandFit?.score||0).toFixed(3)} price=${priceAttractiveness.toFixed(2)} ad=${adReach.toFixed(2)} sales=${salesEffectiveness.toFixed(2)} dist=${distributionCoverage.toFixed(2)} PULL=${pull.toFixed(4)}`);
