...
export const BrandDB = {
  async create({ teamId, name, targetSegment, frameQuality = 3, wheelsQuality = 3, drivetrainQuality = 3, brakesQuality = 3, suspensionQuality = 3, seatQuality = 3, handlebarsQuality = 3, electronicsQuality = 0, rdInvestment = 0 }) {
    ...
    // Quadratic cost: base $50, per component v²×8 + v×15 (matches frontend estimate)
    const unitCost = 50 + components.reduce((sum, q) => sum + q * q * 8 + q * 15, 0);
  ...
  async update(id, updates) {
    ...
    // Quadratic cost: base $50, per component v²×8 + v×15 (matches frontend estimate)
    const unitCost = 50 + components.reduce((s, q) => s + q * q * 8 + q * 15, 0);
...
export const ResultDB = {
  async create(teamId, quarter, results) {
    const r = results;
    // Delete any existing result for this team+quarter to prevent duplicates
    try { await sql`DELETE FROM quarterly_results WHERE team_id = ${teamId} AND quarter = ${quarter}`; } catch(e) {}
    const res = await sql`
