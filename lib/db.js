...
export const UserDB = {
  ...
  async setInstructor(id, isInstructor) {
    const result = await sql`
      UPDATE users SET is_instructor = ${isInstructor}, updated_at = CURRENT_TIMESTAMP
      WHERE id = ${id} RETURNING *
    `;
    return result.rows[0];
  },
  ...
  async getAll() {
    const result = await sql`SELECT id, email, name, is_admin, is_instructor, auth_provider, created_at FROM users ORDER BY created_at DESC`;
    return result.rows;
  }
};
...
// ==================== GAME DB ====================
export const GameDB = {
  async create({ name, code, instructorId, marketScenario = 'bikes', maxTeams = 6, settings = {} }) {
    const result = await sql`
      INSERT INTO games (name, code, instructor_id, market_scenario, max_teams, settings)
      VALUES (${name}, ${code}, ${instructorId}, ${marketScenario}, ${maxTeams}, ${JSON.stringify(settings)})
      RETURNING *
    `;
    return result.rows[0];
  },
  ...
  async findByInstructor(instructorId) {
    const result = await sql`
      ...
      GROUP BY g.id
      HAVING g.instructor_id = ${instructorId}
...
// ==================== TEAM DB ====================
export const TeamDB = {
  async create({ gameId, name, logoEmoji = '🚲' }) {
    const result = await sql`
      INSERT INTO teams (game_id, name, logo_emoji)
      VALUES (${gameId}, ${name}, ${logoEmoji})
      RETURNING *
    `;
    return result.rows[0];
  },
  ...
  async getTeamForUser(userId, gameId) {
    const result = await sql`
      SELECT t.* FROM teams t
      JOIN team_members tm ON tm.team_id = t.id
      WHERE tm.user_id = ${userId} AND t.game_id = ${gameId}
    `;
    return result.rows[0] || null;
  }
};
...
export const TeamMemberDB = {
  ...
  async getUserGames(userId) {
    const result = await sql`
      SELECT g.*, t.id as team_id, t.name as team_name, tm.role
...
export const SegmentDB = {
  ...
  async seedDefaults(gameId) {
    const segments = [
      { name: 'Worker', desc: 'Daily commuters who need reliable, comfortable bikes for getting to work', priceMin: 600, priceMax: 1000, potentialLatam: 3000, potentialEurope: 5000, potentialApac: 4000, growth: 0.05, priceSens: 0.25, performance: 0.10, durability: 0.25, style: 0.05, comfort: 0.25, lightweight: 0.05, customization: 0.05 },
      { name: 'Recreation', desc: 'Weekend riders who enjoy cycling for fun and fitness', priceMin: 700, priceMax: 1200, potentialLatam: 4000, potentialEurope: 6000, potentialApac: 5000, growth: 0.08, priceSens: 0.15, performance: 0.10, durability: 0.15, style: 0.20, comfort: 0.20, lightweight: 0.10, customization: 0.10 },
      { name: 'Youth', desc: 'Young riders and students looking for trendy, affordable bikes', priceMin: 500, priceMax: 900, potentialLatam: 5000, potentialEurope: 4000, potentialApac: 6000, growth: 0.10, priceSens: 0.30, performance: 0.05, durability: 0.10, style: 0.30, comfort: 0.10, lightweight: 0.05, customization: 0.10 },
      { name: 'Mountain', desc: 'Adventure seekers who need rugged, high-performance trail bikes', priceMin: 900, priceMax: 1500, potentialLatam: 2000, potentialEurope: 4000, potentialApac: 3000, growth: 0.06, priceSens: 0.10, performance: 0.30, durability: 0.20, style: 0.05, comfort: 0.05, lightweight: 0.15, customization: 0.15 },
      { name: 'Speed', desc: 'Competitive cyclists focused on performance and lightweight racing bikes', priceMin: 1000, priceMax: 1800, potentialLatam: 1500, potentialEurope: 3500, potentialApac: 2500, growth: 0.04, priceSens: 0.05, performance: 0.30, durability: 0.10, style: 0.10, comfort: 0.05, lightweight: 0.30, customization: 0.10 }
    ];

    for (const s of segments) {
      await sql`
        INSERT INTO market_segments (game_id, name, description, price_sensitivity, performance_weight, durability_weight, style_weight, comfort_weight, lightweight_weight, customization_weight, potential_demand_latam, potential_demand_europe, potential_demand_apac, min_price, max_price, growth_rate)
        VALUES (${gameId}, ${s.name}, ${s.desc}, ${s.priceSens}, ${s.performance}, ${s.durability}, ${s.style}, ${s.comfort}, ${s.lightweight}, ${s.customization}, ${s.potentialLatam}, ${s.potentialEurope}, ${s.potentialApac}, ${s.priceMin}, ${s.priceMax}, ${s.growth})
      `;
    }
  }
};
...
export const BrandDB = {
  async create({ teamId, name, targetSegment, frameQuality = 3, wheelsQuality = 3, drivetrainQuality = 3, brakesQuality = 3, suspensionQuality = 3, seatQuality = 3, handlebarsQuality = 3, electronicsQuality = 0, rdInvestment = 0 }) {
    // Calculate unit cost and overall quality
    ...
    const result = await sql`
      INSERT INTO brands (team_id, name, target_segment, frame_quality, wheels_quality, drivetrain_quality, brakes_quality, suspension_quality, seat_quality, handlebars_quality, electronics_quality, overall_quality, unit_cost, rd_investment)
  ...
  async findByTeam(teamId) {
    const result = await sql`SELECT * FROM brands WHERE team_id = ${teamId} AND is_active = true ORDER BY name`;
    return result.rows;
  },
  ...
  async update(id, updates) {
    ...
    const result = await sql`
      UPDATE brands SET
        ...
        frame_quality = COALESCE(${updates.frameQuality}, frame_quality),
        wheels_quality = COALESCE(${updates.wheelsQuality}, wheels_quality),
        drivetrain_quality = COALESCE(${updates.drivetrainQuality}, drivetrain_quality),
        brakes_quality = COALESCE(${updates.brakesQuality}, brakes_quality),
        suspension_quality = COALESCE(${updates.suspensionQuality}, suspension_quality),
        seat_quality = COALESCE(${updates.seatQuality}, seat_quality),
        handlebars_quality = COALESCE(${updates.handlebarsQuality}, handlebars_quality),
        electronics_quality = COALESCE(${updates.electronicsQuality}, electronics_quality),
  ...
  async deactivate(id) {
    await sql`UPDATE brands SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = ${id}`;
  }
};
...
export const ResultDB = {
  async create(teamId, quarter, results) {
    const r = results;
    const res = await sql`
      INSERT INTO quarterly_results (team_id, quarter, total_demand, units_sold, units_produced, stockouts, market_share_primary, market_share_secondary, revenue, cost_of_goods, gross_profit, advertising_expense, salesforce_expense, distribution_expense, internet_expense, rd_expense, admin_expense, operating_profit, net_income, cash_flow, ending_cash, brand_satisfaction, ad_satisfaction, price_satisfaction, overall_satisfaction, financial_performance, market_performance, marketing_effectiveness, investment_in_future, creation_of_wealth, balanced_scorecard)
      VALUES (${teamId}, ${quarter}, ${r.totalDemand || 0}, ${r.unitsSold || 0}, ${r.unitsProduced || 0}, ${r.stockouts || 0}, ${r.marketSharePrimary || 0}, ${r.marketShareSecondary || 0}, ${r.revenue || 0}, ${r.costOfGoods || 0}, ${r.grossProfit || 0}, ${r.advertisingExpense || 0}, ${r.salesforceExpense || 0}, ${r.distributionExpense || 0}, ${r.internetExpense || 0}, ${r.rdExpense || 0}, ${r.adminExpense || 0}, ${r.operatingProfit || 0}, ${r.netIncome || 0}, ${r.cashFlow || 0}, ${r.endingCash || 0}, ${r.brandSatisfaction || 0}, ${r.adSatisfaction || 0}, ${r.priceSatisfaction || 0}, ${r.overallSatisfaction || 0}, ${r.financialPerformance || 0}, ${r.marketPerformance || 0}, ${r.marketingEffectiveness || 0}, ${r.investmentInFuture || 0}, ${r.creationOfWealth || 0}, ${r.balancedScorecard || 0})
      RETURNING *
    `;
    return res.rows[0];
  },
